...
select distinct count(policy_holder) from co_sandbox_datos.fact_vigentes_total_historico where accountable_period = '202606';

select distinct count(policy_holder) from vigentes_filtrado where accountable_period = '202606';
...
WHERE accountable_period = '202606'
...
/* =========================
    ...
    /* v2 — tres cambios sobre la regla original:

       (a) SOLO SE COMPARA LA INTERSECCIÓN: la regla devuelve NULL (= fuera del
           universo) si CUALQUIERA de las dos fuentes no tiene el dato. Consistencia
           mide si dos cifras cuadran; si solo existe una, no hay nada que medir —
           esa ausencia la reportan completitud y la cobertura, no esta regla.
           La versión anterior era asimétrica: excluía a quien no tenía dato en
           Pirani pero marcaba incumplimiento a quien no lo tenía en CDP, así que
           comparaba conjuntos de tamaños muy distintos.

       (b) Un 0 se trata como "sin dato" en AMBOS lados. Pirani entrega el sin-dato
           como 0, no como NULL (230.452 de 357.140 registros del corte 202606).
           Si negocio confirma que un 0 en CDP es un valor real y no una ausencia,
           basta con quitar `OR rb.total_ingresos = 0` de la segunda condición.

       (c) La tolerancia pasa de ±1 peso (coincidencia exacta, demasiado estricta
           entre dos fuentes distintas) a una tolerancia RELATIVA del 5 %.
           Ajustar el 0.05 al umbral que defina negocio.

       NOTA: el orden de los WHEN importa. Los chequeos de `= 0` van ANTES de la
       comparación porque `0.05 * 0 = 0` haría que solo un match exacto pasara. */
    CASE
        WHEN rb.ingresos_pirani IS NULL OR rb.ingresos_pirani = 0 THEN NULL
        WHEN rb.total_ingresos  IS NULL OR rb.total_ingresos  = 0 THEN NULL
        WHEN ABS(rb.total_ingresos - rb.ingresos_pirani) <= 0.05 * rb.ingresos_pirani THEN 1
        ELSE 0
    END AS rg_consistencia_ingresos,

    CASE
        WHEN rb.egresos_pirani IS NULL OR rb.egresos_pirani = 0 THEN NULL
        WHEN rb.total_egresos  IS NULL OR rb.total_egresos  = 0 THEN NULL
        WHEN ABS(rb.total_egresos - rb.egresos_pirani) <= 0.05 * rb.egresos_pirani THEN 1
        ELSE 0
...
UNION ALL
    ...
    UNION ALL
    /* v2 — consistencia fila a fila (mismo patrón AVG(...)*100 que completitud y
       validez) en vez de la razón entre SUMAS del portafolio. La razón de sumas
       no tiene cota: cuando la desviación supera el 100 %, el GREATEST(0, ...) la
       aplasta a 0 y un −20 % se ve igual que un −1090 %. Además un solo cliente
       grande puede decidir el resultado de todo el indicador.
       El filtro `rg_consistencia_* IS NOT NULL` deja fuera a quienes no tienen
       base de comparación en Pirani: el porcentaje se calcula solo sobre los
       clientes efectivamente comparables. REPORTAR SIEMPRE LA COBERTURA (el
       COUNT del universo) junto al porcentaje: no es lo mismo un 100 % sobre
       10 clientes que sobre 80.000. */
    SELECT
        tipo_persona,
        'total_ingresos' AS atributo,
        'consistencia' AS dimension,
        AVG(rg_consistencia_ingresos::DECIMAL(18,6)) * 100 AS porcentaje
    FROM tmp_reglas_reporte_base
    WHERE tipo_persona IN (1,2)
      AND declara_sarlaft = 1
    ...
    SELECT
        ...
        'consistencia' AS dimension,
        AVG(rg_consistencia_egresos::DECIMAL(18,6)) * 100 AS porcentaje
    FROM tmp_reglas_reporte_base
    WHERE tipo_persona IN (1,2)
      AND declara_sarlaft = 1
      AND rg_consistencia_egresos IS NOT NULL
    GROUP BY tipo_persona
    /* =========================
       OPORTUNIDAD
       ========================= */
    ...
    GROUP BY tipo_persona
    /* =========================
       COBERTURA Y PONDERADA DE LA CONSISTENCIA (v2)
       Consistencia es la unica dimension que necesita una SEGUNDA FUENTE, asi que
       su porcentaje no se puede leer solo: hay que saber sobre cuanta poblacion se
       calculo. Estas dos metricas se calculan sobre TODA la poblacion (sin filtrar
       declara_sarlaft ni la existencia de contraparte):

         cobertura  = % de clientes que tienen con que compararse
         ponderada  = % de clientes que coinciden, sobre el total
                      (equivale a consistencia x cobertura; los no comparables
                       cuentan como "no verificados")

       Se dejan FUERA del bloque SFC a proposito: alli las dimensiones son las
       definidas por la Superintendencia y no admiten codigos nuevos.
       ========================= */
    UNION ALL
    SELECT
        tipo_persona,
        'total_ingresos' AS atributo,
        'cobertura' AS dimension,
        AVG(CASE WHEN rg_consistencia_ingresos IS NOT NULL THEN 1 ELSE 0 END::DECIMAL(18,6)) * 100
    FROM tmp_reglas_reporte_base
    WHERE tipo_persona IN (1,2)
    GROUP BY tipo_persona
    UNION ALL
    SELECT
        ...
        'total_egresos',
        'cobertura',
        AVG(CASE WHEN rg_consistencia_egresos IS NOT NULL THEN 1 ELSE 0 END::DECIMAL(18,6)) * 100
    FROM tmp_reglas_reporte_base
    WHERE tipo_persona IN (1,2)
    GROUP BY tipo_persona
    UNION ALL
    SELECT
        tipo_persona,
        'total_ingresos',
        'consistencia_ponderada',
        AVG(COALESCE(rg_consistencia_ingresos, 0)::DECIMAL(18,6)) * 100
    FROM tmp_reglas_reporte_base
    WHERE tipo_persona IN (1,2)
    GROUP BY tipo_persona
    UNION ALL
    SELECT
        ...
        'total_egresos',
        'consistencia_ponderada',
        AVG(COALESCE(rg_consistencia_egresos, 0)::DECIMAL(18,6)) * 100
    FROM tmp_reglas_reporte_base
    WHERE tipo_persona IN (1,2)
    GROUP BY tipo_persona
)
SELECT
    ...
    ROUND(MAX(CASE WHEN dimension = 'cobertura' THEN porcentaje END), 2) AS pct_cobertura_consistencia,
    ROUND(MAX(CASE WHEN dimension = 'consistencia_ponderada' THEN porcentaje END), 2) AS pct_consistencia_ponderada,
    ...
    /* cumple_consistencia_90 NO debe leerse solo: un 100 % sobre 10 clientes lo
       pone en 1. Esta bandera avisa cuando la cobertura es demasiado baja para
       que el porcentaje signifique algo. Umbral 5 % ajustable por negocio. */
    CASE WHEN MAX(CASE WHEN dimension = 'cobertura' THEN porcentaje END) < 5
         THEN 1 ELSE 0 END AS alerta_cobertura_baja,
...
-----CALIDAD CON FORMATO SFC------

CREATE TABLE tmp_reporte_sfc_calidad_datos AS
WITH metricas AS (
    /* =========================
       COMPLETITUD
       ========================= */
